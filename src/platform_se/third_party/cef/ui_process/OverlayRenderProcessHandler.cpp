  ...
  CefRefPtr<CefV8Context> context)
{
  ...
  pObject->SetValue(m_coreObjectName, m_pCoreObject,
                    V8_PROPERTY_ATTRIBUTE_NONE);
}

void OverlayRenderProcessHandler::OnContextReleased(
  CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame,
  CefRefPtr<CefV8Context> context)
{
  TP_UNUSED(browser);
  ...
  CefProcessId sourceProcess, CefRefPtr<CefProcessMessage> message)
{
  ...
  if (message->GetName() == "browser-event") {
    ...
    for (const auto& callback : m_pEventsHandler->GetCallbacks()) {
      if (callback.first == functionName &&
          callback.second.first->GetBrowser()->IsSame(browser)) {
...
CefRefPtr<CefV8Value> OverlayRenderProcessHandler::ConvertValue(
  const CefRefPtr<CefValue>& apValue)
{
  switch (apValue->GetType()) {
    ...
    case VTYPE_LIST: {
      ...
      auto pConvertedList =
        CefV8Value::CreateArray(static_cast<int>(pOriginalList->GetSize()));

      for (size_t i = 0; i < pOriginalList->GetSize(); ++i) {
        pConvertedList->SetValue(
          static_cast<int>(i),
          ConvertValue(pOriginalList->GetValue(static_cast<int>(i))));
      }
    ...
    case VTYPE_DICTIONARY: {
      ...
      for (const auto& key : keys) {
        pConvertedDict->SetValue(key,
                                 ConvertValue(pOriginalDict->GetValue(key)),
                                 V8_PROPERTY_ATTRIBUTE_NONE);
      }
...
void OverlayRenderProcessHandler::CreateEvent(
  const std::string& acEventName) const
{
  m_pCoreObject->SetValue(
    acEventName, CefV8Value::CreateFunction(acEventName, m_pEventsHandler),
    V8_PROPERTY_ATTRIBUTE_NONE);
}

void OverlayRenderProcessHandler::CreateFunction(
  const std::string& acFunctionName) const
{
  m_pCoreObject->SetValue(
    acFunctionName,
    CefV8Value::CreateFunction(acFunctionName, m_pOverlayHandler),
    V8_PROPERTY_ATTRIBUTE_NONE);
}
}
