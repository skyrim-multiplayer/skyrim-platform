...
bool EventsV8Handler::Execute(const CefString& acName,
                              ...
                              CefString& aException)
{
  if (acName == "on") {
    if (acArguments.size() < 2) {
      ...
      m_callbacks.emplace(
        std::make_pair(acArguments[0]->GetStringValue().ToString(),
                       std::make_pair(m_pContext, acArguments[1])));
    }
    ...
    if (acArguments.empty()) {
      aException = "Invalid number of arguments";
    } else if (!acArguments[0]->IsString() ||
               (acArguments.size() != 1 && !acArguments[1]->IsFunction())) {
      aException = "Invalid arguments";
    } else {
      for (auto it = m_callbacks.begin(); it != m_callbacks.end();) {
        if (it->first == acArguments[0]->GetStringValue().ToString() &&
            it->second.first->IsSame(m_pContext) &&
            (acArguments.size() == 1 ||
             it->second.second->IsSame(acArguments[1]))) {
