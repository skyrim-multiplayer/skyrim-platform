...
bool CDXShaderFactory::CreatePixelShader(
  ...
  Microsoft::WRL::ComPtr<ID3D11PixelShader>& pixelShader)
{
  ...
  HRESULT result = CompileShaderFromData(
  ...
  if (SUCCEEDED(result)) {
    ...
    result = pDevice->CreatePixelShader(shaderBuffer->GetBufferPointer(),
  ...
  } else if (result == E_NOINTERFACE && precompiledFile &&
             precompiledFile->GetBufferSize()) {
    // Create the vertex shader from the buffer.
    result = pDevice->CreatePixelShader(precompiledFile->GetBuffer(),
...
bool CDXShaderFactory::CreateVertexShader(
  CDXD3DDevice* device, CDXShaderFile* sourceFile,
  CDXShaderFile* precompiledFile, D3D11_INPUT_ELEMENT_DESC* polygonLayout,
  int numElements, Microsoft::WRL::ComPtr<ID3D11VertexShader>& vertexShader,
  Microsoft::WRL::ComPtr<ID3D11InputLayout>& layout)
{
  ...
  if (SUCCEEDED(result)) {
    ...
    result = pDevice->CreateInputLayout(
      polygonLayout, numElements, shaderBuffer->GetBufferPointer(),
      shaderBuffer->GetBufferSize(), &layout);
    if (FAILED(result)) {
      _ERROR("%s - Failed to create input layout for vertex shader",
             __FUNCTION__);
      return false;
    }
  } else if (result == E_NOINTERFACE && precompiledFile &&
    ...
    // Create the vertex input layout.
    result = pDevice->CreateInputLayout(
      polygonLayout, numElements, precompiledFile->GetBuffer(),
      precompiledFile->GetBufferSize(), layout.ReleaseAndGetAddressOf());
    if (FAILED(result)) {
      _ERROR("%s - Failed to create input layout for vertex shader",
             __FUNCTION__);
      return false;
...
bool CDXShaderFactory::CreateGeometryShader(
  CDXD3DDevice* device, CDXShaderFile* sourceFile,
  CDXShaderFile* precompiledFile,
...
void CDXShaderFactory::OutputShaderErrorMessage(
  Microsoft::WRL::ComPtr<ID3DBlob>& errorMessage, std::stringstream& output)
{
