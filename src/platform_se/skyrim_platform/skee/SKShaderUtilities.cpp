...
void GetShaderProperty(NiAVObject* node, OverrideVariant* value)
{
  ...
  if (geometry) {
    ...
    if (value->key >= OverrideVariant::kParam_ControllersStart &&
        value->key <= OverrideVariant::kParam_ControllersEnd) {
      ...
      if (controllerIndex != -1) {
        ...
        NiTimeController* controller =
          ni_cast(shaderProperty->m_controller, NiTimeController);
        while (controller) {
        ...
        if (foundController) {
          switch (value->key) {
            case OverrideVariant::kParam_ControllerFrequency:
              PackValue<float>(value, value->key, value->index,
                               &foundController->m_fFrequency);
              break;
            case OverrideVariant::kParam_ControllerPhase:
              PackValue<float>(value, value->key, value->index,
                               &foundController->m_fPhase);
              break;
            case OverrideVariant::kParam_ControllerStartTime:
              PackValue<float>(value, value->key, value->index,
                               &foundController->m_fLoKeyTime);
              break;
            case OverrideVariant::kParam_ControllerStopTime:
              PackValue<float>(value, value->key, value->index,
                               &foundController->m_fHiKeyTime);
              break;

              // Special cases
            case OverrideVariant::kParam_ControllerStartStop: {
              float val = 0.0;
              PackValue<float>(value, value->key, value->index, &val);
              break;
            } break;
            default:
              _MESSAGE("Unknown controller key %d %s", value->key,
                       node->m_name);
              shaderError = true;
              break;
          }
    ...
    if (ni_is_type(shaderProperty->GetRTTI(), BSEffectShaderProperty)) {
      BSEffectShaderMaterial* material =
        (BSEffectShaderMaterial*)shaderProperty->material;
      switch (value->key) {
        case OverrideVariant::kParam_ShaderEmissiveColor:
          PackValue<NiColorA>(value, value->key, value->index,
                              &material->emissiveColor);
          break;
        case OverrideVariant::kParam_ShaderEmissiveMultiple:
          PackValue<float>(value, value->key, value->index,
                           &material->emissiveMultiple);
          break;
        default:
...
#ifdef _DEBUG
      _MESSAGE("Applied EffectShader property %d %X to %s", value->key,
               value->data.u, node->m_name);
#endif
    } else if (ni_is_type(shaderProperty->GetRTTI(),
      ...
      switch (value->key) {
        case OverrideVariant::kParam_ShaderEmissiveColor:
          PackValue<NiColor>(value, value->key, value->index,
                             lightingShader->emissiveColor);
          break;
        case OverrideVariant::kParam_ShaderEmissiveMultiple:
          PackValue<float>(value, value->key, value->index,
                           &lightingShader->emissiveMultiple);
          break;
        case OverrideVariant::kParam_ShaderAlpha:
          PackValue<float>(value, value->key, value->index, &material->alpha);
          break;
        ...
        case OverrideVariant::kParam_ShaderTexture: {
          if (value->index < BSTextureSet::kNumTextures) {
            BSFixedString texture =
              material->textureSet->GetTexturePath(value->index);
            PackValue<BSFixedString>(value, value->key, value->index,
                                     &texture);
          }
        } break;
        ...
        case OverrideVariant::kParam_ShaderTintColor: {
          if (material->GetShaderType() ==
                BSShaderMaterial::kShaderType_FaceGenRGBTint ||
              material->GetShaderType() ==
                BSShaderMaterial::kShaderType_HairTint) {
            BSLightingShaderMaterialFacegenTint* tintedMaterial =
              (BSLightingShaderMaterialFacegenTint*)material;
            PackValue<NiColor>(value, value->key, value->index,
                               &tintedMaterial->tintColor);
          }
        } break;
        default:
          _MESSAGE("Unknown lighting shader key %d %s", value->key,
                   node->m_name);
          shaderError = true;
          break;
      }
#ifdef _DEBUG
      _MESSAGE("Applied LightingShader property %d %X to %s", value->key,
               value->data.u, node->m_name);
#endif
    }
  } else {
...
void NIOVTaskUpdateTexture::Run()
{
  if (m_geometry) {
    BSShaderProperty* shaderProperty =
      niptr_cast<BSShaderProperty>(m_geometry->m_spEffectState);
    if (!shaderProperty) {
      ...
      if (m_index < BSTextureSet::kNumTextures) {
        ...
        for (UInt32 i = 0; i < BSTextureSet::kNumTextures; i++) {
          newTextureSet->SetTexturePath(
            i, material->textureSet->GetTexturePath(i));
        }
        newTextureSet->SetTexturePath(m_index,
                                      m_texture->AsBSFixedString().c_str());
...
void SetShaderProperty(NiAVObject* node, OverrideVariant* value,
                       bool immediate)
{
  BSGeometry* geometry = node->GetAsBSGeometry();
  if (geometry) {
    ...
    if (value->key >= OverrideVariant::kParam_ControllersStart &&
      ...
      if (controllerIndex != -1) {
        ...
        if (foundController) {
          switch (value->key) {
            case OverrideVariant::kParam_ControllerFrequency:
              UnpackValue(&foundController->m_fFrequency, value);
              return;
              break;
            case OverrideVariant::kParam_ControllerPhase:
              UnpackValue(&foundController->m_fPhase, value);
              return;
              break;
            case OverrideVariant::kParam_ControllerStartTime:
              UnpackValue(&foundController->m_fLoKeyTime, value);
              return;
              break;
            case OverrideVariant::kParam_ControllerStopTime:
              UnpackValue(&foundController->m_fHiKeyTime, value);
              return;
              break;
    ...
    BSEffectShaderProperty* effectShader =
      ni_cast(shaderProperty, BSEffectShaderProperty);
    if (effectShader) {
      BSEffectShaderMaterial* material =
        (BSEffectShaderMaterial*)shaderProperty->material;
      switch (value->key) {
        case OverrideVariant::kParam_ShaderEmissiveColor:
          UnpackValue(&material->emissiveColor, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderEmissiveMultiple:
          UnpackValue(&material->emissiveMultiple, value);
          return;
          break;
        default:
          _MESSAGE("Unknown shader key %d %s", value->key, node->m_name);
    ...
    if (lightingShader) {
      BSLightingShaderMaterial* material =
        (BSLightingShaderMaterial*)lightingShader->material;
      switch (value->key) {
        case OverrideVariant::kParam_ShaderEmissiveColor:
          UnpackValue(lightingShader->emissiveColor, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderEmissiveMultiple:
          UnpackValue(&lightingShader->emissiveMultiple, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderAlpha:
          UnpackValue(&material->alpha, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderGlossiness:
          UnpackValue(&material->glossiness, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderSpecularStrength:
          UnpackValue(&material->specularStrength, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderLightingEffect1:
          UnpackValue(&material->lightingEffect1, value);
          return;
          break;
        case OverrideVariant::kParam_ShaderLightingEffect2:
          UnpackValue(&material->lightingEffect2, value);
          return;
          break;
        ...
        case OverrideVariant::kParam_ShaderTexture: {
          ...
          if (immediate) {
            if (value->index >= 0 &&
                value->index < BSTextureSet::kNumTextures) {
              ...
              newTextureSet->SetTexturePath(value->index,
                                            texture.AsBSFixedString().c_str());
              material->SetTextureSet(newTextureSet);

              NiPointer<NiTexture> newTexture;
              LoadTexture(texture.c_str(), 1, newTexture, false);

              NiTexturePtr* targetTexture =
                GetTextureFromIndex(material, value->index);
              if (targetTexture) {
                *targetTexture = newTexture;
              }
            ...
            g_task->AddTask(new NIOVTaskUpdateTexture(
              geometry, value->index, g_stringTable.GetString(texture)));
          }
          return;
        } break;
        case OverrideVariant::kParam_ShaderTextureSet: {
            ...
            if (immediate) {
              ...
              for (UInt32 i = 0; i < BSTextureSet::kNumTextures; i++) {
                const char* texturePath =
                  textureSet->textureSet.GetTexturePath(i);
                newTextureSet->SetTexturePath(i, texturePath);
              }
              material->ReleaseTextures();
              material->SetTextureSet(newTextureSet);
              ...
              CALL_MEMBER_FN(BSTaskPool::GetSingleton(), SetNiGeometryTexture)
            (geometry, textureSet);
          }
          return;
        } break;
        case OverrideVariant::kParam_ShaderTintColor: {
          ...
          if (material->GetShaderType() !=
              ...
              material->GetShaderType() !=
                BSShaderMaterial::
                  kShaderType_HairTint) // if(CALL_MEMBER_FN(lightingShader,
                                        // HasFlags)(0x0A))
          {
            BSLightingShaderMaterialFacegenTint* tintedMaterial =
              CreateFacegenTintMaterial();
        ...
        default:
          _ERROR("Unknown lighting shader key %d %s", value->key,
                 node->m_name);
          return;
          break;
      }
#ifdef _DEBUG
      _DMESSAGE("Applied LightingShader property %d %X to %s", value->key,
                value->data.u, node->m_name);
#endif
    }
  } else {
...
public:
  ...
  {
    if (pForm) {
      BGSBipedObjectForm* pBip =
        DYNAMIC_CAST(pForm, TESForm, BGSBipedObjectForm);
      if (pBip) {
...
TESForm* GetWornForm(Actor* thisActor, UInt32 mask)
{
  ...
  ExtraContainerChanges* pContainerChanges =
    static_cast<ExtraContainerChanges*>(
      thisActor->extraData.GetByType(kExtraData_ContainerChanges));
...
void VisitAllWornItems(Actor* thisActor, UInt32 mask,
                       std::function<void(InventoryEntryData*)> functor)
{
  struct VisitEquippedStack
  {
    VisitEquippedStack(FormMatcher& matcher,
                       std::function<void(InventoryEntryData*)>& results)
      : m_matcher(matcher)
      , m_results(results)
    {
    }
    ...
    bool Accept(InventoryEntryData* pEntryData)
    {
      if (pEntryData) {
        ...
        if (pExtendList && m_matcher.Matches(pEntryData->type)) {
          for (ExtendDataList::Iterator it = pExtendList->Begin(); !it.End();
               ++it) {
            BaseExtraList* pExtraDataList = it.Get();

            if (pExtraDataList) {
              if (pExtraDataList->HasType(kExtraData_Worn) ||
...
void VisitEquippedNodes(
  Actor* actor, UInt32 slotMask,
  std::function<void(TESObjectARMO*, TESObjectARMA*, NiAVObject*, bool)>
    functor)
{
  std::unordered_set<TESObjectARMO*> equippedSlots;
  VisitAllWornItems(actor, slotMask, [&](InventoryEntryData* itemData) {
    TESObjectARMO* armor = itemData->type->formType == TESObjectARMO::kTypeID
      ? static_cast<TESObjectARMO*>(itemData->type)
      : nullptr;
    if (armor) {
  ...
  for (auto& armor : equippedSlots) {
    for (UInt32 i = 0; i < armor->armorAddons.count; i++) {
      ...
      if (armor->armorAddons.GetNthItem(i, arma)) {
        if (arma->isValidRace(
              actor->race)) { // Only search AAs that fit this race
          VisitArmorAddon(actor, armor, arma,
                          [&](bool isFirstPerson, NiNode* skeletonRoot,
...
void VisitArmorAddon(Actor* actor, TESObjectARMO* armor, TESObjectARMA* arma,
                     std::function<void(bool, NiNode*, NiAVObject*)> functor)
{
  ...
  for (UInt32 i = 0; i <= 1; i++) {
    if (skeletonRoot[i]) {
      ...
      if (root) {
        ...
        if (rootNode) {
          BSFixedString addonName(
            addonString); // Find the Armor name from the root

          // DFS search for the node by name, then traverse all siblings incase
          // the same armor appears twice
          NiAVObject* armorNode =
            skeletonRoot[i]->GetObjectByName(&addonName.data);
          ...
          _DMESSAGE(
            "%s - Failed to locate addon node for Armor: %08X on Actor: %08X",
            __FUNCTION__, armor->formID, actor->formID);
        }
#endif
      }
    }
...
bool ResolveAnyForm(SKSESerializationInterface* intfc, UInt32 handle,
                    UInt32* newHandle)
{
  if (((handle & 0xFF000000) >> 24) != 0xFF) {
    ...
    if (!refr ||
        (refr &&
         (refr->flags & TESForm::kFlagIsDeleted) == TESForm::kFlagIsDeleted)) {
      return false;
    }
    *newHandle = handle;
  }
...
bool ResolveAnyHandle(SKSESerializationInterface* intfc, UInt64 handle,
                      UInt64* newHandle)
{
  if (((handle & 0xFF000000) >> 24) != 0xFF) {
    // Skip if handle is no longer valid.
...
SKEEFixedString GetSanitizedPath(const SKEEFixedString& path)
{
  ...
  fullPath = std::regex_replace(
    fullPath,
    std::regex(R"(.*?[^\s]textures\\|^textures\\)",
               std::regex_constants::icase),
    ""); // Remove everything before and including the textures path root

  return fullPath;
}

NiTexturePtr* GetTextureFromIndex(BSLightingShaderMaterial* material,
                                  UInt32 index)
{
  switch (index) {
    case 0:
    ...
    case 2: {
      ...
      } else if (material->GetShaderType() ==
                 BSShaderMaterial::kShaderType_GlowMap) {
        return &static_cast<BSLightingShaderMaterialGlowmap*>(material)
                  ->glowMap;
      } else {
    ...
    case 3: {
      if (material->GetShaderType() == BSShaderMaterial::kShaderType_FaceGen) {
        return &static_cast<BSLightingShaderMaterialFacegen*>(material)->unkA8;
                 ...
                 BSShaderMaterial::kShaderType_Parallax) {
        return &static_cast<BSLightingShaderMaterialParallax*>(material)
                  ->unkA0;
      } else if (material->GetShaderType() ==
                 BSShaderMaterial::kShaderType_ParallaxOcc) {
        return &static_cast<BSLightingShaderMaterialParallaxOcc*>(material)
                  ->unkA0;
      }
    } break;
    case 4: {
      if (material->GetShaderType() == BSShaderMaterial::kShaderType_Eye) {
        return &static_cast<BSLightingShaderMaterialEye*>(material)->unkA0;
                 ...
                 BSShaderMaterial::kShaderType_EnvironmentMap) {
        return &static_cast<BSLightingShaderMaterialEnvmap*>(material)->unkA0;
      } else if (material->GetShaderType() ==
                 BSShaderMaterial::kShaderType_MultilayerParallax) {
        return &static_cast<BSLightingShaderMaterialMultiLayerParallax*>(
                  material)
                  ->unkA8;
      }
    } break;
    ...
    case 6: {
      if (material->GetShaderType() == BSShaderMaterial::kShaderType_FaceGen) {
        return &static_cast<BSLightingShaderMaterialFacegen*>(material)
                  ->renderedTexture;
      } else if (material->GetShaderType() ==
                 BSShaderMaterial::kShaderType_MultilayerParallax) {
        return &static_cast<BSLightingShaderMaterialMultiLayerParallax*>(
...
void DumpNodeChildren(NiAVObject* node)
{
  ...
  if (node->m_extraDataLen > 0) {
    ...
    for (UInt16 i = 0; i < node->m_extraDataLen; i++) {
      _MESSAGE("{%s} {%s} {%X}", node->m_extraData[i]->GetRTTI()->name,
               node->m_extraData[i]->m_pcName, node);
    }
    gLog.Outdent();*/
  }
  ...
  if (niNode && niNode->m_children.m_emptyRunStart > 0) {
    ...
    for (int i = 0; i < niNode->m_children.m_emptyRunStart; i++) {
      ...
      if (object) {
        ...
        BSGeometry* geometry = object->GetAsBSGeometry();
        if (geometry) {
          _MESSAGE("{%s} {%s} {%X} - Geometry", object->GetRTTI()->name,
          ...
          if (shaderProperty) {
            ...
            if (lightingShader) {
              ...
              for (int i = 0; i < BSTextureSet::kNumTextures; ++i) {
                const char* texturePath =
                  material->textureSet->GetTexturePath(i);
                if (!texturePath) {
                  continue;
                }
          ...
          _MESSAGE("{%s} {%s} {%X}", object->GetRTTI()->name, object->m_name,
                   object);
        }
      }
    }
