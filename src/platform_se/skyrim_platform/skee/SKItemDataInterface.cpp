...
bool ModifiedItemFinder::Accept(InventoryEntryData* pEntryData)
{
  ...
  while (pExtraDataList) {
    ...
    if ((m_identifier.type & ModifiedItemIdentifier::kTypeRank) ==
        ModifiedItemIdentifier::kTypeRank) {
      if (pExtraDataList->HasType(kExtraData_Rank)) {
        ExtraRank* extraRank =
          static_cast<ExtraRank*>(pExtraDataList->GetByType(kExtraData_Rank));
        if (m_identifier.rankId == extraRank->rank)
          isMatch = true;
      }
    }

    if ((m_identifier.type & ModifiedItemIdentifier::kTypeUID) ==
        ModifiedItemIdentifier::kTypeUID) {
      if (pExtraDataList->HasType(kExtraData_UniqueID)) {
        ExtraUniqueID* extraUID = static_cast<ExtraUniqueID*>(
          pExtraDataList->GetByType(kExtraData_UniqueID));
        if (m_identifier.uid == extraUID->uniqueId &&
            m_identifier.ownerForm == extraUID->ownerFormId)
          isMatch = true;
      }
    }

    if ((m_identifier.type & ModifiedItemIdentifier::kTypeSlot) ==
        ModifiedItemIdentifier::kTypeSlot) {
      if (isWorn) {
        TESObjectARMO* armor =
          DYNAMIC_CAST(pEntryData->type, TESForm, TESObjectARMO);
        ...
        // Is not an armor, we don't have a slot mask search, and it's equipped
          ...
          if ((m_identifier.weaponSlot ==
                 ModifiedItemIdentifier::kHandSlot_Left &&
               hasWornLeft) ||
              (m_identifier.weaponSlot ==
                 ModifiedItemIdentifier::kHandSlot_Right &&
               hasWorn))
            isMatch = true;
        }
      }
...
bool ResolveModifiedIdentifier(TESObjectREFR* reference,
                               ModifiedItemIdentifier& identifier,
                               ModifiedItem& itemData)
{
  if (!reference)
    return false;
  ...
  ExtraContainerChanges* pContainerChanges =
    static_cast<ExtraContainerChanges*>(
...
std::shared_ptr<ItemAttributeData> ModifiedItem::GetAttributeData(
  TESObjectREFR* reference, bool makeUnique, bool allowNewEntry, bool isSelf,
  UInt32* idOut)
{
  ExtraRank* rank = NULL;
  ...
  if (pExtraData->HasType(kExtraData_UniqueID)) {
    uniqueId =
      static_cast<ExtraUniqueID*>(pExtraData->GetByType(kExtraData_UniqueID));
    ...
    if (pContainerChanges) {
      ...
      if (containerData) {
        ...
        uniqueId = static_cast<ExtraUniqueID*>(
          pExtraData->GetByType(kExtraData_UniqueID));
      }
    }
  ...
  if (rank) {
    ...
    if (!data && uniqueId && allowNewEntry) {
      ...
      data = g_itemDataInterface.CreateData(
        rank->rank, uniqueId ? uniqueId->uniqueId : 0,
        uniqueId ? uniqueId->ownerFormId : 0, pForm->formID);
...
std::shared_ptr<ItemAttributeData> ItemDataInterface::GetExistingData(
  TESObjectREFR* reference, ModifiedItemIdentifier& identifier)
{
  ModifiedItem foundData;
  if (ResolveModifiedIdentifier(reference, identifier, foundData)) {
    return foundData.GetAttributeData(reference, false, false,
                                      identifier.IsSelf());
  }
...
NIOVTaskUpdateItemDye::NIOVTaskUpdateItemDye(
  Actor* actor, ModifiedItemIdentifier& identifier, UInt32 flags, bool forced,
  LayerFunctor layerFunctor)
{
...
void NIOVTaskUpdateItemDye::Run()
{
  ...
  if (actor) {
    ...
    if (ResolveModifiedIdentifier(actor, m_identifier, foundData)) {
      auto data =
        foundData.GetAttributeData(actor, false, true, m_identifier.IsSelf());
      if (!data && !m_forced) {
      ...
      TESObjectARMO* armor =
        DYNAMIC_CAST(foundData.pForm, TESForm, TESObjectARMO);
      if (armor) {
        for (UInt32 i = 0; i < armor->armorAddons.count; i++) {
          TESObjectARMA* arma = NULL;
          if (armor->armorAddons.GetNthItem(i, arma)) {
            VisitArmorAddon(actor, armor, arma,
                            [&](bool isFirstPerson, NiAVObject* rootNode,
                                NiAVObject* parent) {
                              g_tintMaskInterface.ApplyMasks(
                                actor, isFirstPerson, armor, arma, parent,
                                m_flags, data, m_layerFunctor);
                            });
          }
        }
      }
...
UInt32 ItemDataInterface::GetItemUniqueID(TESObjectREFR* reference,
                                          ModifiedItemIdentifier& identifier,
                                          bool makeUnique)
{
  ModifiedItem foundData;
  if (ResolveModifiedIdentifier(reference, identifier, foundData)) {
    UInt32 id = 0;
    auto data = foundData.GetAttributeData(reference, makeUnique, true,
                                           identifier.IsSelf(), &id);
    if (data) {
      return id;
    }
  }
...
void ItemDataInterface::SetItemTextureLayerColor(UInt32 uniqueID,
                                                 SInt32 textureIndex,
                                                 SInt32 layerIndex,
                                                 UInt32 color)
{
...
void ItemDataInterface::SetItemTextureLayerBlendMode(UInt32 uniqueID,
                                                     SInt32 textureIndex,
                                                     SInt32 layerIndex,
                                                     SKEEFixedString blendMode)
{
  auto data = GetData(uniqueID);
  if (data) {
    data->m_tintData[textureIndex].m_blendMap[layerIndex] =
      g_stringTable.GetString(blendMode);
  }
}

void ItemDataInterface::SetItemTextureLayerType(UInt32 uniqueID,
                                                SInt32 textureIndex,
                                                SInt32 layerIndex, UInt32 type)
{
  auto data = GetData(uniqueID);
  if (data) {
    data->m_tintData[textureIndex].m_typeMap[layerIndex] =
      static_cast<UInt8>(type);
  }
}

void ItemDataInterface::SetItemTextureLayerTexture(UInt32 uniqueID,
                                                   SInt32 textureIndex,
                                                   SInt32 layerIndex,
                                                   SKEEFixedString texture)
{
  auto data = GetData(uniqueID);
  if (data) {
    data->m_tintData[textureIndex].m_textureMap[layerIndex] =
      g_stringTable.GetString(texture);
  }
}

UInt32 ItemDataInterface::GetItemTextureLayerColor(UInt32 uniqueID,
                                                   SInt32 textureIndex,
                                                   SInt32 layerIndex)
{
  UInt32 color = 0;
...
SKEEFixedString ItemDataInterface::GetItemTextureLayerBlendMode(
  UInt32 uniqueID, SInt32 textureIndex, SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
  if (data) {
    auto& layerData = data->m_tintData.find(textureIndex);
...
UInt32 ItemDataInterface::GetItemTextureLayerType(UInt32 uniqueID,
                                                  SInt32 textureIndex,
                                                  SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
...
SKEEFixedString ItemDataInterface::GetItemTextureLayerTexture(
  UInt32 uniqueID, SInt32 textureIndex, SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
  if (data) {
...
void ItemDataInterface::ClearItemTextureLayerColor(UInt32 uniqueID,
                                                   SInt32 textureIndex,
                                                   SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
...
void ItemDataInterface::ClearItemTextureLayerBlendMode(UInt32 uniqueID,
                                                       SInt32 textureIndex,
                                                       SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
...
void ItemDataInterface::ClearItemTextureLayerType(UInt32 uniqueID,
                                                  SInt32 textureIndex,
                                                  SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
...
void ItemDataInterface::ClearItemTextureLayerTexture(UInt32 uniqueID,
                                                     SInt32 textureIndex,
                                                     SInt32 layerIndex)
{
  auto data = GetData(uniqueID);
...
void ItemDataInterface::ClearItemTextureLayer(UInt32 uniqueID,
                                              SInt32 textureIndex)
{
  auto data = GetData(uniqueID);
  if (data) {
...
std::shared_ptr<ItemAttributeData> ItemDataInterface::GetData(UInt32 rankId)
{
  ...
  auto it =
    std::find_if(m_data.begin(), m_data.end(), [&](ItemAttribute& item) {
      if (item.rank == rankId)
        return true;
      return false;
    });
...
std::shared_ptr<ItemAttributeData> ItemDataInterface::CreateData(
  UInt32 rankId, UInt16 uid, UInt32 ownerId, UInt32 formId)
{
  EraseByRank(rankId);

  std::shared_ptr<ItemAttributeData> data =
    std::make_shared<ItemAttributeData>();
  Lock();
...
bool ItemDataInterface::UpdateUIDByRank(UInt32 rankId, UInt16 uid,
                                        UInt32 formId)
{
  SimpleLocker lock(&m_lock);
...
bool ItemDataInterface::UpdateUID(UInt16 oldId, UInt32 oldFormId, UInt16 newId,
                                  UInt32 newFormId)
{
  SimpleLocker lock(&m_lock);
...
bool ItemDataInterface::EraseByUID(UInt32 uid, UInt32 formId)
{
  ...
  if (it != m_data.end()) {
    auto eventDispatcher = static_cast<EventDispatcher<SKSEModCallbackEvent>*>(
      g_messaging->GetEventDispatcher(
        SKSEMessagingInterface::kDispatcher_ModEvent));
    if (eventDispatcher) {
      TESForm* form = LookupFormByID((*it).formId);
      SKSEModCallbackEvent evn("NiOverride_Internal_EraseUID", "", (*it).uid,
                               form);
...
void ItemDataInterface::UpdateInventoryItemDye(UInt32 rankId,
                                               TESObjectARMO* armor,
                                               NiAVObject* rootNode)
{
  g_task->AddTask(new NIOVTaskDeferredMask(
    (*g_thePlayer), false, armor, nullptr, rootNode,
    rankId ? g_itemDataInterface.GetData(rankId) : nullptr));
}

void ItemDataInterface::ForEachItemAttribute(
  std::function<void(const ItemAttribute&)> functor)
{
  SimpleLocker lock(&m_lock);
...
void ItemDataInterface::OnAttach(TESObjectREFR* refr, TESObjectARMO* armor,
                                 ...
                                 NiNode* root)
{
  ...
  if (actor) {
    ...
    g_task->AddTask(new NIOVTaskDeferredMask(
      refr, isFirstPerson, armor, addon, object,
      g_itemDataInterface.GetExistingData(actor, identifier)));
  }
}
...
void ItemDataInterface::Revert()
{
  ...
  for (auto& itemAttribute : copy) {
    ...
    if (actor && armor) {
      ...
      m_loadQueue.push_back(new NIOVTaskUpdateItemDye(
        actor, identifier, TintMaskInterface::kUpdate_All, true));
    }
  }
  m_nextRank = 1;
}
...
void ItemAttributeData::TintData::Save(SKSESerializationInterface* intfc,
                                       UInt32 kVersion)
{
  struct DataMap
  {
  ...
  for (auto tint : layerMap) {
    ...
    if (layerData.color)
      overrideFlags |= OverrideFlags::kColor;
    if (layerData.texture)
      overrideFlags |= OverrideFlags::kTextureMap;
    if (layerData.blendMode)
      overrideFlags |= OverrideFlags::kBlendMap;
    if (layerData.type)
      overrideFlags |= OverrideFlags::kTypeMap;
    ...
    if (layerData.blendMode) {
      intfc->WriteRecordData(layerData.blendMode,
                             sizeof(*layerData.blendMode));
    }
    if (layerData.type) {
...
bool ItemAttributeData::TintData::Load(SKSESerializationInterface* intfc,
                                       UInt32 kVersion,
...
void ItemAttributeData::Save(SKSESerializationInterface* intfc,
                             UInt32 kVersion)
{
  if (!m_tintData.empty()) {
...
bool ItemAttributeData::Load(SKSESerializationInterface* intfc,
...
EventResult ItemDataInterface::ReceiveEvent(
  TESUniqueIDChangeEvent* evn,
  EventDispatcher<TESUniqueIDChangeEvent>* dispatcher)
{
  if (evn->oldOwnerFormId != 0) {
    g_itemDataInterface.UpdateUID(evn->oldUniqueId, evn->oldOwnerFormId,
                                  evn->newUniqueId, evn->newOwnerFormId);
  }
...
void ItemDataInterface::Save(SKSESerializationInterface* intfc,
                             UInt32 kVersion)
{
...
bool ItemDataInterface::Load(SKSESerializationInterface* intfc,
                             UInt32 kVersion, const StringIdMap& stringTable)
{
  UInt32 type, length, version;
  bool error = false;

  // Resolve formIds from any prior queues (if there are entries here its
  // because Revert queued them)
  for (auto& task : m_loadQueue) {
    // If the task was queued from a previous save we need to resolve the
    // actor's formId first
    UInt32 newItemForm = 0;
    if (!intfc->ResolveFormId(task->m_formId, &newItemForm)) {
      _WARNING("%s - actor %08X could not be found, skipping dye update",
               __FUNCTION__, task->m_formId);
      continue;
    }
    ...
    if (intfc->GetNextRecordInfo(&type, &version, &length)) {
      switch (type) {
        case 'IDAT': {
          ...
          UInt64 ownerHandle = 0, itemHandle = 0, newOwnerHandle = 0,
                 newItemHandle = 0;
          if (version >= kSerializationVersion2) {
            ...
            if (!ResolveAnyForm(intfc, ownerForm, &newOwnerForm)) {
              _WARNING("%s - owner %08X could not be found, skipping",
                       __FUNCTION__, ownerForm);
              continue;
            }
            ...
            if (!intfc->ResolveFormId(itemForm, &newItemForm)) {
              _WARNING("%s - item %08X could not be found, skipping",
                       __FUNCTION__, itemForm);
              continue;
            }
            ...
            if (!ResolveAnyHandle(intfc, ownerHandle, &newOwnerHandle)) {
              _WARNING(
                "%s - owner handle %016llX could not be found, skipping",
                __FUNCTION__, ownerHandle);
              continue;
            }
            if (!intfc->ResolveHandle(itemHandle, &newItemHandle)) {
              _WARNING("%s - item handle %016llX could not be found, skipping",
                       __FUNCTION__, itemHandle);
              continue;
            }
          }
          ...
          if (data->Load(intfc, version, stringTable)) {
            _ERROR(
              "%s - Failed to load item data for owner %016llX item %016llX",
              __FUNCTION__, ownerHandle, itemHandle);
            error = true;
            return error;
          }
          ...
          if (rankId != kInvalidRank) {
            ...
            Actor* actor = DYNAMIC_CAST(ownerForm, TESForm, Actor);
            TESObjectARMO* armor =
              DYNAMIC_CAST(itemForm, TESForm, TESObjectARMO);
            if (actor && armor) {
              ModifiedItemIdentifier identifier;
              identifier.SetSlotMask(armor->bipedObject.GetSlotMask());
              m_loadQueue.push_back(new NIOVTaskUpdateItemDye(
                actor, identifier, TintMaskInterface::kUpdate_All, false));
            }
          }
        } break;
        default:
          _ERROR("Error loading unexpected chunk type %08X (%.4s)", type,
                 &type);
          error = true;
          break;
      }
  ...
  std::sort(m_loadQueue.begin(), m_loadQueue.end(),
            [](NIOVTaskUpdateItemDye* a, NIOVTaskUpdateItemDye* b) {
  ...
  std::unique_copy(m_loadQueue.begin(), m_loadQueue.end(),
                   std::inserter(uniqueTasks, uniqueTasks.end()),
                   [](NIOVTaskUpdateItemDye* a, NIOVTaskUpdateItemDye* b) {
                     return a->GetActor() == b->GetActor() &&
                       a->GetSlotMask() == b->GetSlotMask();
                   });
