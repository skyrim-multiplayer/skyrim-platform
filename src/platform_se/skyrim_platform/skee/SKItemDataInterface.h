...
struct ModifiedItem
{
  ...
  operator bool() const { return (pForm && pExtraData); }

  std::shared_ptr<ItemAttributeData> GetAttributeData(
    TESObjectREFR* reference, bool makeUnique = true,
    bool allowNewEntry = true, bool allowSelf = false, UInt32* idOut = NULL);
};
...
{
public:
  ModifiedItemFinder(ModifiedItemIdentifier& identifier)
    : m_identifier(identifier)
  {
  }
  bool Accept(InventoryEntryData* pEntryData);
  ModifiedItem& Found() { return m_found; };
...
public:
  ...
  public:
    ...
    bool empty() const
    {
      return m_textureMap.empty() && m_colorMap.empty() &&
        m_blendMap.empty() && m_typeMap.empty();
    }
...
public:
  ...
  virtual EventResult ReceiveEvent(
    ...
    EventDispatcher<TESUniqueIDChangeEvent>* dispatcher) override;
  ...
  virtual UInt32 GetItemUniqueID(TESObjectREFR* reference,
                                 ModifiedItemIdentifier& identifier,
                                 bool makeUnique);
  virtual void SetItemTextureLayerColor(UInt32 uniqueID, SInt32 textureIndex,
                                        SInt32 layerIndex, UInt32 color);
  virtual void SetItemTextureLayerType(UInt32 uniqueID, SInt32 textureIndex,
                                       SInt32 layerIndex, UInt32 type);
  virtual void SetItemTextureLayerBlendMode(UInt32 uniqueID,
                                            SInt32 textureIndex,
                                            SInt32 layerIndex,
                                            SKEEFixedString blendMode);
  virtual void SetItemTextureLayerTexture(UInt32 uniqueID, SInt32 textureIndex,
                                          SInt32 layerIndex,
                                          SKEEFixedString texture);

  virtual UInt32 GetItemTextureLayerColor(UInt32 uniqueID, SInt32 textureIndex,
                                          SInt32 layerIndex);
  virtual UInt32 GetItemTextureLayerType(UInt32 uniqueID, SInt32 textureIndex,
                                         SInt32 layerIndex);
  virtual SKEEFixedString GetItemTextureLayerBlendMode(UInt32 uniqueID,
                                                       SInt32 textureIndex,
                                                       SInt32 layerIndex);
  virtual SKEEFixedString GetItemTextureLayerTexture(UInt32 uniqueID,
                                                     SInt32 textureIndex,
                                                     SInt32 layerIndex);

  virtual void ClearItemTextureLayerColor(UInt32 uniqueID, SInt32 textureIndex,
                                          SInt32 layerIndex);
  virtual void ClearItemTextureLayerType(UInt32 uniqueID, SInt32 textureIndex,
                                         SInt32 layerIndex);
  virtual void ClearItemTextureLayerBlendMode(UInt32 uniqueID,
                                              SInt32 textureIndex,
                                              SInt32 layerIndex);
  virtual void ClearItemTextureLayerTexture(UInt32 uniqueID,
                                            SInt32 textureIndex,
                                            SInt32 layerIndex);
  ...
  std::shared_ptr<ItemAttributeData> GetExistingData(
    TESObjectREFR* reference, ModifiedItemIdentifier& identifier);
  std::shared_ptr<ItemAttributeData> CreateData(UInt32 rankId, UInt16 uid,
                                                UInt32 ownerId, UInt32 formId);
  std::shared_ptr<ItemAttributeData> GetData(UInt32 rankId);
  bool UpdateUIDByRank(UInt32 rankId, UInt16 uid, UInt32 formId);
  bool UpdateUID(UInt16 oldId, UInt32 oldFormId, UInt16 newId,
                 UInt32 newFormId);
  bool EraseByRank(UInt32 rankId);
  bool EraseByUID(UInt32 uid, UInt32 formId);

  void UpdateInventoryItemDye(UInt32 rankId, TESObjectARMO* armor,
                              NiAVObject* rootNode);
...
{
public:
  NIOVTaskUpdateItemDye(Actor* actor, ModifiedItemIdentifier& identifier,
                        UInt32 flags, bool forced,
                        LayerFunctor layerFunctor = LayerFunctor());
  virtual void Run();
