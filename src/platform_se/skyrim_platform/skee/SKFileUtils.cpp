...
#pragma comment(lib, "Shlwapi.lib") // For PathCombineA
  ...
  while (!visit.empty()) {
    ...
    if (character) {
      for (UInt32 i = 0; i < character->leveledList.length; i++) {
        ...
        if (form) {
          TESLevCharacter* levCharacter =
            DYNAMIC_CAST(form, TESForm, TESLevCharacter);
...
void ForEachMod(std::function<void(ModInfo*)> functor)
{
  ...
  {
  public:
    ActiveModVisitor(std::function<void(ModInfo*)> fn)
      : m_function(fn)
    {
    }
