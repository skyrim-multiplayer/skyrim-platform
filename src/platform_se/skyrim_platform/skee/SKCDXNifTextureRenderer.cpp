...
#include "SKCDXBSShaderResource.h"
...
bool CDXNifTextureRenderer::ApplyMasksToTexture(
  CDXD3DDevice* device, NiPointer<NiTexture> texture,
  std::map<SInt32, MaskData>& masks, const BSFixedString& name,
  NiPointer<NiTexture>& output)
{
  ...
  if (!SetTexture(device, rendererData->resourceView,
  ...
  for (const auto& mask : masks) {
    ...
    if (a > 0.0f) {
      ...
      AddLayer((texture && texture->rendererData)
                 ? texture->rendererData->resourceView
                 : nullptr,
               static_cast<TextureType>(mask.second.textureType),
               mask.second.technique.c_str(), XMFLOAT4(r, g, b, a));
    }
  ...
  // Will promote caching of textures for live-editing as this will hold onto
  // them until cleanup
  m_textures = textures;
  ...
  NiTexture::RendererData* sourceData =
    new NiTexture::RendererData(GetWidth(), GetHeight());
