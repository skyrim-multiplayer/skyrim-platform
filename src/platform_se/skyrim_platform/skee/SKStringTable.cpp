...
StringTableItem StringTable::GetString(const SKEEFixedString& str)
{
  ...
  if (it != m_table.end()) {
    ...
    StringTableItem item = std::shared_ptr<SKEEFixedString>(
      new SKEEFixedString(str), DeleteStringEntry);
...
void StringTable::Save(const SKSESerializationInterface* intfc,
                       UInt32 kVersion)
{
...
bool StringTable::Load(const SKSESerializationInterface* intfc,
                       UInt32 kVersion, StringIdMap& stringTable)
{
  bool error = false;
  ...
  if (kVersion >= kSerializationVersion3) {
    ...
    for (UInt32 i = 0; i < totalStrings; i++) {
      ...
      if (stringLength > 0 &&
          !intfc->ReadRecordData(stringName, stringLength)) {
        _ERROR("%s - Error loading string of length %d", __FUNCTION__,
               stringLength);
        error = true;
        return error;
      }
...
template <typename T>
bool Serialization::WriteData(const SKSESerializationInterface* intfc,
                              const T* data)
{
...
template <>
bool Serialization::WriteData<SKEEFixedString>(
  const SKSESerializationInterface* intfc, const SKEEFixedString* str)
{
...
template <>
bool Serialization::ReadData<SKEEFixedString>(
  const SKSESerializationInterface* intfc, SKEEFixedString* str)
{
  UInt16 len = 0;
