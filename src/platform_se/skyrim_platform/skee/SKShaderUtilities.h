...
{
public:
  NIOVTaskUpdateTexture(NiPointer<BSGeometry> geometry, UInt32 index,
                        StringTableItem texture)
    : m_geometry(geometry)
    , m_index(index)
    , m_texture(texture)
  {
...
{
public:
  NiExtraDataFinder::NiExtraDataFinder(BSFixedString name)
    : m_name(name)
    , m_data(NULL)
  {
  }
...
void VisitEquippedNodes(
  ...
  std::function<void(TESObjectARMO*, TESObjectARMA*, NiAVObject*, bool)>
    functor);
void VisitAllWornItems(Actor* thisActor, UInt32 slotMask,
