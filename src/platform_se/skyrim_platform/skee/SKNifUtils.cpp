...
bool SaveRenderedDDS(NiTexture* pkTexture, const char* pcFileName)
{
  ...
  if (resource) {
    texture = rendererData->texture;
             ...
             rendererData->resourceView) // Didn't have texture directly but
                                         // still has resource view, acquire it
  {
  ...
  res = DirectX::SaveToDDSFile(si.GetImages(), si.GetImageCount(),
                               si.GetMetadata(), DirectX::DDS_FLAGS_NONE,
                               fileName);
  if (FAILED(res)) {
    delete[] fileName;
...
BSGeometry* GetHeadGeometry(Actor* actor, UInt32 partType)
{
  ...
  if (faceNode && actorBase) {
    ...
    if (facePart) {
      NiAVObject* headNode =
        faceNode->GetObjectByName(&facePart->partName.data);
      if (headNode) {
...
NiAVObject* GetObjectByHeadPart(BSFaceGenNiNode* faceNode,
...
SKSETaskRefreshTintMask::SKSETaskRefreshTintMask(Actor* actor,
                                                 BSFixedString ddsPath)
  : m_ddsPath(ddsPath)
{
    ...
    if (shaderProperty) {
      if (ni_is_type(shaderProperty->GetRTTI(), BSLightingShaderProperty)) {
        BSLightingShaderProperty* lightingShader =
          static_cast<BSLightingShaderProperty*>(shaderProperty);
        BSLightingShaderMaterial* material =
          static_cast<BSLightingShaderMaterial*>(shaderProperty->material);
        if (material) {
    ...
    if (ni_is_type(shaderProperty->GetRTTI(), BSLightingShaderProperty)) {
      ...
      if (material) {
        if (material->GetShaderType() ==
            BSShaderMaterial::kShaderType_FaceGen) {
          BSLightingShaderMaterialFacegen* maskedMaterial =
            (BSLightingShaderMaterialFacegen*)material;
          SaveRenderedDDS(
            niptr_cast<NiRenderedTexture>(maskedMaterial->renderedTexture),
            filePath.data);
        }
      }
    }
...
std::pair<BGSTextureSet*, BGSHeadPart*> GetTextureSetForPartByName(
  TESNPC* npc, BSFixedString partName)
{
...
SKSETaskExportHead::SKSETaskExportHead(Actor* actor, BSFixedString nifPath,
                                       BSFixedString ddsPath)
  : m_nifPath(nifPath)
  , m_ddsPath(ddsPath)
{
...
void SKSETaskExportHead::Run()
{
  ...
  if (animationData) {
    ...
    for (UInt32 t = BSFaceGenAnimationData::kKeyframeType_Expression;
         t <= BSFaceGenAnimationData::kKeyframeType_Phoneme; t++) {
  ...
  for (UInt32 i = 0; i < faceNode->m_children.m_size; i++) {
    ...
    if (NiGeometry* geometry = object->GetAsNiGeometry()) {
      NiGeometryData* geometryData =
        niptr_cast<NiGeometryData>(geometry->m_spModelData);
      NiGeometryData* newGeometryData = NULL;
      if (geometryData)
        CALL_MEMBER_FN(geometryData, DeepCopy)((NiObject**)&newGeometryData);

      NiProperty* trishapeEffect =
        niptr_cast<NiProperty>(geometry->m_spEffectState);
      NiPointer<NiProperty> newTrishapeEffect;
      if (trishapeEffect)
        CALL_MEMBER_FN(trishapeEffect, DeepCopy)
      ((NiObject**)&newTrishapeEffect);

      NiProperty* trishapeProperty =
        niptr_cast<NiProperty>(geometry->m_spPropertyState);
      NiPointer<NiProperty> newTrishapeProperty;
      if (trishapeProperty)
        CALL_MEMBER_FN(trishapeProperty, DeepCopy)
      ((NiObject**)&newTrishapeProperty);

      NiSkinInstance* skinInstance =
        niptr_cast<NiSkinInstance>(geometry->m_spSkinInstance);
      ...
      if (skinInstance) {
        ...
        UInt32 numBones = 0;
        NiSkinData* skinData =
          niptr_cast<NiSkinData>(skinInstance->m_spSkinData);
        ...
        NiSkinPartition* skinPartition =
          niptr_cast<NiSkinPartition>(skinInstance->m_spSkinPartition);
        NiSkinPartition* newSkinPartition = NULL;
        if (skinPartition)
          CALL_MEMBER_FN(skinPartition, DeepCopy)
        ((NiObject**)&newSkinPartition);
        ...
        if (numBones > 0) {
          newSkinInstance->m_ppkBones =
            (NiAVObject**)Heap_Allocate(numBones * sizeof(NiAVObject*));
      ...
      if (NiTriShape* trishape = geometry->GetAsNiTriShape()) {
        NiTriShape* newTrishape =
          NiTriShape::Create(static_cast<NiTriShapeData*>(newGeometryData));
        ...
        if (tristrips) {
          NiTriStrips* newTristrips =
            NiTriStrips::Create(static_cast<NiTriShapeData*>(newGeometryData));
          newGeometryData->DecRef();
      ...
      if (newGeometry) {
        auto textureData =
          GetTextureSetForPartByName(actorBase, newGeometry->m_name);
        if (textureData.first && textureData.second) {
          BSShaderProperty* shaderProperty =
            niptr_cast<BSShaderProperty>(newGeometry->m_spEffectState);
          if (shaderProperty) {
            if (ni_is_type(shaderProperty->GetRTTI(),
                           BSLightingShaderProperty)) {
              BSLightingShaderProperty* lightingShader =
                static_cast<BSLightingShaderProperty*>(shaderProperty);
              BSLightingShaderMaterial* material =
                static_cast<BSLightingShaderMaterial*>(
                  shaderProperty->material);
              if (material && material->textureSet) {
                for (UInt32 i = 0; i < BGSTextureSet::kNumTextures; i++)
                  material->textureSet->SetTexturePath(
                    i, textureData.first->textureSet.GetTexturePath(i));
          ...
          BSShaderProperty* originalShaderProperty =
            niptr_cast<BSShaderProperty>(geometry->m_spEffectState);
          if (originalShaderProperty) {
            if (ni_is_type(originalShaderProperty->GetRTTI(),
                           BSLightingShaderProperty)) {
              BSLightingShaderProperty* lightingShader =
                static_cast<BSLightingShaderProperty*>(originalShaderProperty);
              BSLightingShaderMaterial* material =
                static_cast<BSLightingShaderMaterial*>(
                  originalShaderProperty->material);
              if (material) {
                if (material->GetShaderType() ==
                    BSShaderMaterial::kShaderType_FaceGen) {
                  BSLightingShaderMaterialFacegen* maskedMaterial =
                    static_cast<BSLightingShaderMaterialFacegen*>(material);
                  SaveRenderedDDS(niptr_cast<NiRenderedTexture>(
                                    maskedMaterial->renderedTexture),
                                  m_ddsPath.data);
                }
              }
            }
      ...
      if (skinInstance) {
        newSkinInstance =
          skinInstance->Clone(); // Clonned instance starts at 0
        newSkinInstance->m_pkRootParent = skinnedNode;
        ...
        if (numBones > 0) {
          ...
          if (!g_exportSkinToBone) {
            for (UInt32 i = 0; i < newSkinData->m_uiBones; i++) {
              ...
              newSkinData->m_pkBoneData[i].m_kSkinToBone.pos =
      ...
      if (trishape) {
        ...
        if (dynamicShape) {
          ...
          if (dynamicShape->pDynamicData) {
            ...
            xData->pDynamicData =
              NiAllocate(trishape->numVertices * sizeof(__m128));
            memcpy(xData->pDynamicData, dynamicShape->pDynamicData,
                   trishape->numVertices * sizeof(__m128));
...
void GetTriangleIndices(NiTriStripsData* strips, UInt16 i, UInt16 v0,
                        UInt16 v1, UInt16 v2)
{
  UInt16 usTriangles;
  UInt16 usStrip = 0;
