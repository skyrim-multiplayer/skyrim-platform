...
#include "SKtinyxml2.h"

#include "SKCDXD3DDevice.h"
#include "SKCDXNifPixelShaderCache.h"
...
void TintMaskInterface::CreateTintsFromData(
  TESObjectREFR* refr,
  ...
  UInt32& flags)
{
  ...
  if (refr->baseForm && refr->baseForm->formType == TESNPC::kTypeID) {
    ...
    skinColor = ((UInt32)actorBase->color.red << 16) |
      ((UInt32)actorBase->color.green << 8) | (UInt32)actorBase->color.blue;
...
NIOVTaskDeferredMask::NIOVTaskDeferredMask(
  TESObjectREFR* refr, bool isFirstPerson, TESObjectARMO* armor,
  TESObjectARMA* addon, NiAVObject* object, ItemAttributeDataPtr overrides)
{
...
void NIOVTaskDeferredMask::Run()
{
  ...
  if (refrForm && armorForm) {
    ...
    TESObjectARMA* addon =
      addonForm ? DYNAMIC_CAST(addonForm, TESForm, TESObjectARMA) : nullptr;

    if (refr && armor) {
      g_tintMaskInterface.ApplyMasks(refr, m_firstPerson, armor, addon,
                                     m_object, TintMaskInterface::kUpdate_All,
                                     m_overrides);
    }
  }
}

void TintMaskInterface::ApplyMasks(TESObjectREFR* refr, bool isFirstPerson,
                                   TESObjectARMO* armor, TESObjectARMA* addon,
                                   NiAVObject* rootNode, UInt32 flags,
                                   ...
                                   LayerFunctor layerFunctor)
{
  ...
  VisitObjects(rootNode, [&](NiAVObject* object) {
    ...
    if (mask.object = object->GetAsBSGeometry()) {
      auto textureData = ni_cast(
        object->GetExtraData(BSFixedString("MASKT").data), NiStringsExtraData);
      if (textureData) {
      ...
      auto colorData =
        ni_cast(object->GetExtraData(BSFixedString("MASKC").data),
                NiIntegersExtraData);
      if (colorData) {
        for (SInt32 i = 0; i < colorData->m_size && i < colorData->m_size;
             ++i) {
          mask.colorData[i] = colorData->m_data[i];
        }
      }

      auto alphaData = ni_cast(
        object->GetExtraData(BSFixedString("MASKA").data), NiFloatsExtraData);
      if (alphaData) {
        for (SInt32 i = 0; i < alphaData->m_size && i < alphaData->m_size;
             ++i) {
  ...
  m_modelMap.ApplyLayers(
    refr, isFirstPerson, armor, addon, rootNode,
    [&](NiPointer<BSGeometry> geom, SInt32 targetIndex, TextureLayer* layer) {
      LayerTarget obj;
      obj.object = geom;
  ...
  for (auto& layer : layerList) {
    NiPointer<BSShaderProperty> shaderProperty =
      niptr_cast<BSShaderProperty>(layer.object->m_spEffectState);
    if (shaderProperty) {
      ...
      if (lightingShader) {
        ...
        CreateTintsFromData(refr, tintMasks, layer, itemOverrideData,
                            changedFlags);
        ...
        if (material->textureSet) {
          texturePath =
            material->textureSet->GetTexturePath(layer.targetIndex);
          if (!texturePath) {
            continue;
          }
        ...
        if (m_maskMap.IsCaching()) {
          renderer =
            m_maskMap.GetRenderTarget(lightingShader, layer.targetIndex);
        }
        if (!renderer) {
          ...
          if (renderer && m_maskMap.IsCaching()) {
            m_maskMap.AddRenderTargetGroup(lightingShader, layer.targetIndex,
                                           renderer);
          }
        }

        char path[512];
        _snprintf_s(path, 512, "RT [%08X][%08X][%08X](%s)", refr->formID,
                    armor->formID, addon ? addon->formID : 0,
                    layer.object->m_name);

        NiPointer<NiTexture> output;
        if (renderer->ApplyMasksToTexture(device.get(), sourceTexture,
                                          tintMasks, path, output)) {
          BSLightingShaderMaterial* newMaterial =
            static_cast<BSLightingShaderMaterial*>(material->Create());
          newMaterial->Copy(material);

          // If the target material has HairTint we should neutralize the color
          // and block it from receiving tint updates But only if theres a
          // layer in the XML that says to do this, this is still missing TODO
          if (newMaterial->GetShaderType() ==
              BSLightingShaderMaterial::kShaderType_HairTint) {
            BSLightingShaderMaterialHairTint* hairMaterial =
              static_cast<BSLightingShaderMaterialHairTint*>(newMaterial);
          ...
          NiTexturePtr* targetTexture =
            GetTextureFromIndex(newMaterial, layer.targetIndex);
          if (targetTexture) {
          ...
          _snprintf_s(texturePath, MAX_PATH, "Layer_%d_%s.dds", i++,
                      layer.object->m_name);
          int len = strlen(texturePath);
          for (int i = 0; i < len; ++i) {
            if (isalpha(texturePath[i]) || isdigit(texturePath[i]) ||
                texturePath[i] == '.' || texturePath[i] == '_')
              continue;
            texturePath[i] = '_';
          }
          ...
          CALL_MEMBER_FN(lightingShader, SetMaterial)
          (newMaterial, 1); // This creates a new material from the one we
                            // created above, so we destroy it after
...
std::shared_ptr<CDXNifTextureRenderer> TintMaskMap::GetRenderTarget(
...
void TintMaskMap::AddRenderTargetGroup(
  BSLightingShaderProperty* key, SInt32 index,
...
TextureLayer* MaskModelMap::GetMask(SKEEFixedString nif,
                                    SKEEFixedString trishape,
...
bool ApplyMaskData(
  MaskTriShapeMap* triShapeMap, NiAVObject* object, const char* nameOverride,
  std::function<void(NiPointer<BSGeometry>, SInt32, TextureLayer*)> functor)
{
  ...
  auto textureMap =
    triShapeMap->GetTextureMap(nameOverride ? nameOverride : object->m_name);
  if (!textureMap) {
    return false;
  }

  NiPointer<NiProperty> shaderProperty =
    niptr_cast<NiProperty>(geometry->m_spEffectState);
  if (!shaderProperty) {
    return false;
  }
...
SKEEFixedString MaskModelMap::GetModelPath(UInt8 gender, bool isFirstPerson,
                                           TESObjectARMO* armor,
                                           TESObjectARMA* arma)
{
  ...
  if (arma) {
    modelPath =
      arma->models[isFirstPerson == true ? 1 : 0][gender].GetModelName();
    if (isFirstPerson &&
        modelPath.length() ==
          0) { // If first person was not found, try third person
      ...
      for (UInt8 fp = 0; fp <= 1; ++fp) {
        for (UInt32 i = 0; i < armor->armorAddons.count; i++) {
          ...
          if (armor->armorAddons.GetNthItem(i, arma) && arma) {
            SKEEFixedString modelPath =
              m_modelMap.GetModelPath(g, fp == 1, armor, arma);
...
void TintMaskInterface::VisitTemplateData(
  TESObjectREFR* refr, TESObjectARMO* armor,
  std::function<void(MaskTriShapeMap*)> functor)
{
  ...
  if (actorBase) {
    ...
    for (UInt32 i = 0; i < armor->armorAddons.count; ++i) {
      ...
      auto shapeMap = m_modelMap.GetTriShapeMap(
        m_modelMap.GetModelPath(gender, false, armor, addon));
      if (shapeMap) {
        functor(shapeMap);
      }
    }
...
void TintMaskInterface::GetTemplateColorMap(TESObjectREFR* refr,
                                            TESObjectARMO* armor,
...
void TintMaskInterface::GetSlotTextureIndexMap(
  TESObjectREFR* refr, TESObjectARMO* armor,
...
void TintMaskInterface::LoadMods()
{
  ...
  FileUtils::GetAllFiles("Data\\SKSE\\Plugins\\NiOverride\\TintData\\",
                         "*.xml", tintFiles);
    ...
    while (object) {
      ...
      while (child) {
        ...
        while (mask) {
          ...
          UInt8 typeNumber =
            static_cast<UInt8>(CDXNifTextureRenderer::TextureType::Mask);
          if (_strnicmp(type, "mask", 4) == 0) {
            typeNumber =
              static_cast<UInt8>(CDXNifTextureRenderer::TextureType::Mask);
          } else if (_strnicmp(type, "normal", 6) == 0) {
            typeNumber =
              static_cast<UInt8>(CDXNifTextureRenderer::TextureType::Normal);
          } else if (_strnicmp(type, "solid", 5) == 0 ||
                     _strnicmp(type, "color", 5) == 0) {
            typeNumber =
              static_cast<UInt8>(CDXNifTextureRenderer::TextureType::Color);
          } else {
            SInt32 typeValue = 0;
...
void TintMaskInterface::OnAttach(TESObjectREFR* refr, TESObjectARMO* armor,
                                 ...
                                 NiNode* root)
{
  ...
  if (((armorMask & addonMask) & (g_tintHairSlot)) != 0) {
    Character* actor = refr->formType == Character::kTypeID
      ? static_cast<Character*>(refr)
      : nullptr;
    TESNPC* actorBase = static_cast<TESNPC*>(refr->baseForm);
    if (actor && actorBase) {
      auto headData = actorBase->headData;
      if (headData) {
        ...
        if (hairColorForm) {
          ...
          NiColorA val;
          val.r =
            static_cast<float>(min((hairColorForm->abgr & 0xFF) * 2, 255)) /
            255.0f;
          val.g = static_cast<float>(
                    min(((hairColorForm->abgr >> 8) & 0xFF) * 2, 255)) /
            255.0f;
          val.b = static_cast<float>(
                    min(((hairColorForm->abgr >> 16) & 0xFF) * 2, 255)) /
            255.0f;
          val.a = 1.0f;
          NiColorA* color = &val;
...
EventResult TintMaskInterface::ReceiveEvent(
    ...
    if (actor && actorBase) {
      ...
      if (headData) {
        ...
        if (hairColorForm) {
          ...
          VisitEquippedNodes(
            actor, g_tintHairSlot,
            [&](TESObjectARMO* armor, TESObjectARMA* arma, NiAVObject* node,
                bool isFP) { UpdateModelHair(node, &color); });
        }
      }
    }
  }
