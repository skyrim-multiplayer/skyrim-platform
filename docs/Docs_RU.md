...
* SkyrimPlatform позволяет изменить реализацию любой консольной команды игры, для подобной модификации вам необходимо получить объект консольной команды, передав имя команды  в метод `findConsoleCommand(comandName)` короткое или длинное.
```typescript
let getAV = findConsoleCommand("GetActorValueInfo");

let getAV = findConsoleCommand("GetAVInfo");
```
* Получив такой объект вы можете изменить короткое (`longName`) или же длинное (`shortName`) имя команды, а также количество принимаемых аргументов (`numArgs`)  и функцию (`execute`) которая будет выполнятся при вызове этой консольной команды через консоль игры.
```typescript
...
* Первым аргументом будет число являющееся ID по которому можно найти `ObjectRefence`  для которого была вызвана консольная команда (Только в том случае если ссылка была указана).
* Остальными параметрами будут аргументы с которыми была вызвана консольная команда которые могут являться строкой или числом.
* Поскольку  игровые функции не доступны в этом потоке, вы можете зарегистрировать метод `once("update")` который выполниться при следующем кадре.
```typescript
